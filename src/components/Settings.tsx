                    ...
                    {!companyInfo.isSubscribed && (
                      <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-slate-400">
                          <FileText size={16} />
                          <span className="text-xs font-medium">{isRTL ? 'تاريخ البدء:' : 'Start Date:'} {new Date(companyInfo.createdAt || Date.now()).toLocaleDateString(isRTL ? 'ar-SA' : 'en-US')}</span>
                        </div>
                        {user.role === 'admin' && (
                          <button 
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // Local state will sync via onSnapshot in App.tsx but we can also set it if we want immediate feedback
                                alert(isRTL ? 'تم تفعيل الاشتراك الاحترافي بنجاح!' : 'Pro subscription activated successfully!');
                              } catch (err) {
                          ...
                          >
                            <CreditCard size={18} />
                            <span>{isRTL ? 'تفعيل النسخة الكاملة' : 'Activate Full Version'}</span>
                          </button>
                        )}
                      </div>
                    )}
